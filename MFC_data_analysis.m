...
%% 二、读取二进制文件，直接转化为数据
% 每两个字节为一个数据，也就是uint32。32位为一个数字。0x0000AABB为一个数
data_init = cell(NUMBER,1);
for ch = 1:NUMBER
    fileID = fopen(whole_file_path{ch},'r');
    if(fileID >3 )
        data_init{ch} = fread(fileID,'uint32','b');
        fclose(fileID);
    end
end
...
data_len = analysis(Ind_row,1); % 减去包头、包尾、时间信息、探测器编号4个数
...
elseif(sum(ENERGY) ~=0)
    ...
    ylabel('Detector NO.');
    zlabel('Count');
    title(['炮号为',num2str(PV),'的 ',num2str(ENERGY),'能量下的测试结果'])
end
