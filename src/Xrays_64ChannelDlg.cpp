	...
	case 2:
		// 炮号检测定时器：检测炮号是否刷新
		{
			...
			if (MeasureMode == 2) {
				...
				for (int num = 0; num < 4; num++) {
					if(connectStatusList[num]) {
						BackSend(num, Order::StartWaveHardTrigger, 12, 0, 1);
					}
...
void CXrays_64ChannelDlg::OnBnClickedStart()
{
	...
	if (strTemp == _T("开始测量")) {
		...
		for (int num = 0; num < 4; num++) {
			if(connectStatusList[num]) {
				BackSend(num, Order::StartWaveSoftTrigger, 12, 0, 1);
			}
			singleLock.Lock();
			if (singleLock.IsLocked()){
				TrigerMode[num] = 1;
...
void CXrays_64ChannelDlg::OnBnClickedAutomeasure()
{
	...
	if (strTemp == _T("自动测量")) {
		...
		if (GetDataStatus) {
			...
			// 发送停止指令，带指令反馈，结束上一次测量结束。
			for (int num = 0; num < 4; num++) {
				if (connectStatusList[num]) BackSend(num, Order::WaveStop, 12, 0, 1);
			}
			sendStopFlag = TRUE;
