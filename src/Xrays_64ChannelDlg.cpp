...
void CXrays_64ChannelDlg::OnConnect()
{
	...
	if (strTemp == _T("连接网络")) {
		...
		GetDlgItem(IDC_Start)->EnableWindow(FALSE);
		GetDlgItem(IDC_AutoMeasure)->EnableWindow(FALSE);
		m_page1.PrintLog(_T("TCP网络(CH1,CH2)已断开"));
	}
	// 恢复各个输入框使能状态
	...
	case 1:
		{
			...
			//状态栏显示
			CString strInfo;
			strInfo.Format(_T("Receieve:CH1=%d,CH2=%d"),
				RECVLength[0], RECVLength[1]);
			...
			if (timer * TIMER_INTERVAL >= MeasureTime){
					...
					&& RECVLength[1] == jsonSetting["RECVLength_CH2"].asInt())
				{
					...
					if (RECVLength[0] + RECVLength[1] > 0) {
						// 打印日志
						CString info = _T("实验数据存储路径：") + saveAsTargetPath;
						m_page1.PrintLog(info);
						info.Format(_T("Receieve Data(byte): CH1 = %d, CH2 = %d"),
							RECVLength[0], RECVLength[1]);
						m_page1.PrintLog(info);
					}
					// 关闭文件指针
